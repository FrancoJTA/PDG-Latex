...
{\normalsize\bfseries ÍNDICE DE ANEXOS\par}
\vspace{\baselineskip}
\noindent\hangindent=1.7em Anexo A.\quad Entrevista semiestructurada aplicada a usuario clave del ERP ISI Mustang \dotfill \pageref{anexo:A}\par
\end{spacing}
\newpage
...
ISI Mustang Bolivia ejecuta proyectos integrales de tipo EPC (Engineering, Procurement and Construction), modalidad en la que la empresa asume de forma completa el diseño de ingeniería, la procura de equipos y materiales, y la construcción de instalaciones para los sectores de gas, petróleo, minería y energía. Para planificar y controlar estos proyectos —avances, certificaciones, recursos, compras y horas trabajadas— la empresa se apoya en un ERP propio, denominado igualmente ISI Mustang, que acumula más de diez años de datos históricos operativos. Sin embargo, dicha información se utiliza únicamente con fines descriptivos y de control inmediato, sin un mecanismo que permita identificar patrones o anticipar comportamientos futuros. Como consecuencia, la detección de desviaciones presupuestarias, retrasos y cuellos de botella ocurre de forma reactiva, limitando la capacidad de gestión proactiva de riesgos en proyectos de alta complejidad técnica y económica.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{perfil/imagenes/ishikawa.png}
...
% --- Títulos de capa ---
\node[titulo] at (0,-0.25)    {Capa 1\\Fuentes\\{\scriptsize(Bronze)}};
\node[titulo] at (3.1,-0.25)  {Capa 2\\Unificación\\{\scriptsize(Silver)}};
\node[titulo] at (6.2,-0.25)  {Capa 3\\Transformación\\{\scriptsize(Gold)}};
\node[titulo] at (9.3,-0.25)  {Capa 4\\Modelos};
\node[titulo] at (12.9,-0.25) {Capa 5\\Servicio};
\node[titulo] at (16.0,-0.25) {Capa 6\\Presentación};

% --- Componentes ---
\node[compex, text width=2.35cm] at (0,-2.55) {\textbf{MySQL}\\Portal anterior};
\node[compex, text width=2.35cm] at (0,-3.75) {\textbf{PostgreSQL}\\ERP ISI Mustang};
\node[comp, text width=2.35cm]   at (0,-4.95) {\textbf{Planillas Excel}\\de control};
...
\node[compex] at (16.0,-2.9){\textbf{NestJS}};
\node[comp]   at (16.0,-4.9){\textbf{Angular}\\Dashboard};
...
Siguiendo la arquitectura Medallion, la capa de fuentes (Bronze) reúne los datos operativos crudos de tres orígenes: el histórico de más de diez años del sistema legado (Portal ISIMustang Bolivia, MySQL), que se utiliza para el entrenamiento; los proyectos activos del ERP nuevo (PostgreSQL), sobre los que se realiza la inferencia; y las planillas Excel de control de proyectos que se llevan fuera del sistema; la capa de unificación (Silver) los consolida y depura mediante DuckDB; la capa de transformación (Gold) realiza la ingeniería de variables con Polars; y la capa de modelos entrena los modelos predictivos para los casos de uso definidos. Posteriormente, la capa de servicio expone los modelos como una API REST con FastAPI, que el backend NestJS del ERP consume para presentar las predicciones en un dashboard de Angular.
...
\quad 2.3 Ingeniería de Datos \\
\quad\quad 2.3.1 Procesos ETL y ELT \\
\quad\quad 2.3.2 Arquitectura Medallion (Bronze, Silver, Gold) \\
\quad\quad 2.3.3 Calidad de Datos \\
\quad 2.4 Analítica Predictiva \\
\quad\quad 2.4.1 Definición y Aplicaciones \\
\quad\quad 2.4.2 Analítica Predictiva en Gestión de Proyectos \\
\quad\quad 2.4.3 Predicción de Desviaciones y Costos \\
\quad 2.5 Gestión de Proyectos EPC \\
\quad\quad 2.5.1 Definición y Características \\
\quad\quad 2.5.2 Certificaciones y Control de Avance \\
\quad\quad 2.5.3 Desviaciones Presupuestarias y de Cronograma \\
\quad 2.6 Ingeniería de Software \\
\quad\quad 2.6.1 Proceso de Desarrollo de Software \\
\quad\quad 2.6.2 Metodología de Desarrollo Iterativa e Incremental \\
\quad\quad 2.6.3 Arquitectura de Software por Capas \\
\quad 2.7 Tecnologías del Stack de Desarrollo \\
\quad\quad 2.7.1 Python y su Ecosistema ML \\
\quad\quad 2.7.2 DuckDB y Polars \\
\quad\quad 2.7.3 FastAPI como Microservicio \\
\quad\quad 2.7.4 NestJS y Angular \\
\quad 2.8 Gestión de Identidad y Acceso \\
\quad\quad 2.8.1 OAuth 2.0 y OpenID Connect \\
\quad\quad 2.8.2 Keycloak \\
\quad 2.9 Model Context Protocol (MCP) \\
\quad\quad 2.9.1 Definición y Arquitectura \\
\quad\quad 2.9.2 Integración de Sistemas Empresariales con Modelos de Lenguaje \\
\quad 2.10 Inteligencia de Negocios \\
\quad\quad 2.10.1 Definición y Reportes Descriptivos \\
\quad\quad 2.10.2 Power BI \\[2pt]
CAPÍTULO III. ANÁLISIS Y DISEÑO DEL SISTEMA PROPUESTO \\
\quad 3.1 Análisis de Datos Históricos \\
\quad\quad 3.1.1 Exploración y Caracterización de las Fuentes de Datos \\
\quad\quad 3.1.2 Identificación de Variables Predictoras \\
\quad\quad 3.1.3 Definición de Casos de Uso Predictivos \\
\quad 3.2 Análisis de Procesos del Portal Anterior \\
\quad\quad 3.2.1 Relevamiento de Procesos del Portal Anterior \\
\quad\quad 3.2.2 Procesos a Reimplementar en el ERP \\
\quad\quad 3.2.3 Defectos y Brechas que Afectan la Calidad de los Datos \\
\quad 3.3 Requerimientos del Sistema \\
\quad\quad 3.3.1 Requerimientos Funcionales \\
\quad\quad 3.3.2 Requerimientos No Funcionales \\
\quad 3.4 Diseño de la Arquitectura \\
\quad\quad 3.4.1 Arquitectura por Capas \\
\quad\quad 3.4.2 Diseño del Pipeline de Datos \\
\quad\quad 3.4.3 Diseño de Modelos ML \\
\quad\quad 3.4.4 Diseño de la API Predictiva \\
\quad\quad 3.4.5 Diseño del Dashboard \\
\quad\quad 3.4.6 Diseño de la Autenticación Centralizada \\
\quad\quad 3.4.7 Diseño de las Integraciones con MCP y Power BI \\[2pt]
CAPÍTULO IV. CONSTRUCCIÓN E IMPLEMENTACIÓN DEL SISTEMA \\
\quad 4.1 Incremento 1: Procesos del Portal Anterior Migrados al ERP \\
\quad 4.2 Incremento 2: Autenticación Centralizada con Keycloak \\
\quad 4.3 Incremento 3: Pipeline de Datos y Entrenamiento de Modelos \\
\quad\quad 4.3.1 Integración DuckDB con MySQL y PostgreSQL \\
\quad\quad 4.3.2 Transformación de Datos con Polars y Feature Engineering \\
\quad\quad 4.3.3 Modelo de Desviación en Certificaciones \\
\quad\quad 4.3.4 Modelo de Sobrecosto de Proyectos \\
\quad\quad 4.3.5 Modelo de Retraso en la Fecha de Cierre \\
\quad 4.4 Incremento 4: Microservicio FastAPI e Integración con el ERP \\
\quad\quad 4.4.1 Desarrollo del Microservicio FastAPI \\
\quad\quad 4.4.2 Endpoints de Predicción en NestJS \\
\quad\quad 4.4.3 Dashboard de Inteligencia Predictiva en Angular \\
\quad 4.5 Incremento 5: Integración con Power BI y MCP \\
\quad\quad 4.5.1 API de Datos para Power BI \\
\quad\quad 4.5.2 Servidor MCP \\
\quad 4.6 Evaluación y Validación \\
\quad\quad 4.6.1 Métricas de Rendimiento de Modelos \\
\quad\quad 4.6.2 Pruebas con Usuarios Clave \\[2pt]
CONCLUSIONES \\
RECOMENDACIONES \\
...
Los fundamentos teóricos del presente trabajo se organizan en siete áreas del conocimiento: sistemas ERP, aprendizaje automático, ingeniería de datos, analítica predictiva, gestión de proyectos EPC, ingeniería de software y tecnologías del stack de desarrollo. Cada área aporta los conceptos necesarios para comprender el problema, justificar el enfoque metodológico y sustentar las decisiones técnicas del módulo propuesto.
...
La ingeniería de datos proporciona los procesos que transforman los datos operativos crudos en conjuntos de datos confiables y listos para el entrenamiento de modelos. \textcite{ReisHousley2022} distinguen dos enfoques para el movimiento y la preparación de datos: el ETL (Extract, Transform, Load), en el cual la información se transforma antes de cargarse en el destino, y el ELT (Extract, Load, Transform), en el cual los datos crudos se cargan primero y la transformación se realiza dentro del propio motor analítico. Para el presente trabajo se adopta el enfoque ELT, dado que permite consolidar los datos del ERP sin transformaciones previas y aprovechar la capacidad de DuckDB como motor analítico embebido para ejecutar las transformaciones sobre los datos ya integrados, simplificando el pipeline y preservando los datos originales para su reprocesamiento.

Para organizar este pipeline se adopta la arquitectura Medallion, un patrón que estructura los datos en tres capas de calidad progresiva —Bronze, Silver y Gold— donde cada capa refina e incrementa el valor de los datos respecto a la anterior \parencite{DatabricksMedallion}. La capa Bronze conserva los datos crudos tal como se extraen de las fuentes; la capa Silver realiza la limpieza, deduplicación y conformado; y la capa Gold entrega conjuntos de datos enriquecidos y agregados, optimizados para el consumo de los modelos de aprendizaje automático. Si bien este patrón se popularizó en entornos de tipo lakehouse, constituye un patrón lógico de organización agnóstico de la herramienta, plenamente aplicable sobre DuckDB y archivos Parquet sin requerir infraestructura especializada. En el módulo propuesto, las capas de fuentes, unificación y transformación de la arquitectura se corresponden, respectivamente, con los niveles Bronze, Silver y Gold de este patrón.

La adopción de un esquema de refinamiento progresivo aporta, además, un mecanismo de control de calidad de los datos: cada capa aplica validaciones y depuraciones que mejoran la confiabilidad de la información antes de alimentar los modelos. Este aspecto resulta especialmente relevante dado que, como se identificó en el análisis FODA, el registro histórico del ERP puede contener inconsistencias; el tratamiento por capas permite detectarlas y corregirlas de forma sistemática antes del entrenamiento.
...
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{|>{\raggedright\arraybackslash}p{6.6cm}|*{5}{>{\centering\arraybackslash}p{1.4cm}|}}
\hline
\textbf{Actividad} & \textbf{Abril} & \textbf{Mayo} & \textbf{Junio} & \textbf{Julio} & \textbf{Agosto} \\
\hline
Elaboración y presentación del perfil & \cellcolor{blue!25}\textbf{X}& & & & \\
\hline
Revisión y aprobación por el tribunal & \cellcolor{blue!25}\textbf{X}& & & & \\
\hline
Elaboración del marco teórico & \cellcolor{blue!25}\textbf{X}& \cellcolor{blue!25}\textbf{X}& & & \\
\hline
Análisis de datos históricos del ERP & \cellcolor{blue!25}\textbf{X}& \cellcolor{blue!25}\textbf{X}& & & \\
\hline
Diseño de la arquitectura y selección de modelos & & \cellcolor{blue!25}\textbf{X}& & & \\
\hline
Construcción del pipeline y entrenamiento de modelos & & & \cellcolor{blue!25}\textbf{X}& & \\
\hline
Integración del módulo y dashboard al ERP & & & & \cellcolor{blue!25}\textbf{X}& \\
\hline
Evaluación y validación con usuarios clave & & & & & \cellcolor{blue!25}\textbf{X}\\
\hline
Redacción del documento final & & & & \cellcolor{blue!25}\textbf{X}& \cellcolor{blue!25}\textbf{X}\\
\hline
Presentación y defensa & & & & & \cellcolor{blue!25}\textbf{X}\\
\hline
\end{tabular}

\vspace{4pt}
{\footnotesize\textbf{Leyenda:} la celda sombreada con \textbf{X} indica el mes en que se ejecuta la actividad.}
\end{table}
...
\addcontentsline{toc}{subsection}{Anexo A. Entrevista a usuario clave del ERP ISI Mustang}
\noindent\textbf{\large Anexo A. Entrevista semiestructurada aplicada a usuario clave del ERP ISI Mustang}\label{anexo:A}

\textbf{Objetivo:} relevar, desde la perspectiva operativa de un usuario clave, las desviaciones y riesgos más frecuentes en la gestión de proyectos de ingeniería, las características y la calidad de los datos históricos registrados en el ERP, y las necesidades y expectativas respecto al módulo de inteligencia predictiva propuesto.

\textbf{Modalidad:} entrevista semiestructurada e individual. El entrevistado otorgó su consentimiento para el registro de la conversación. La información se presenta de forma anonimizada y se utiliza únicamente con fines académicos en el marco del presente proyecto de grado.

\vspace{0.5\baselineskip}
\noindent\begin{tabular}{@{}p{5.4cm}p{9.6cm}@{}}
\textbf{Entrevistado (código):}    & E-01 \\[6pt]
\textbf{Cargo / rol:}              & Jefe de Proyectos \\[6pt]
\textbf{Área o departamento:}      & Gerencia de Proyectos \\[6pt]
\textbf{Antigüedad en la empresa:} & 8 años \\[6pt]
\textbf{Fecha y duración:}         & 12 de junio de 2026 --- 40 minutos \\[6pt]
\end{tabular}

\vspace{0.5\baselineskip}
\noindent\textbf{Bloque I. Contexto del rol y gestión de proyectos}
\begin{enumerate}\setlength{\itemsep}{6pt}\setlength{\parskip}{0pt}
  \item ¿Cuál es su rol y cuáles son sus principales responsabilidades en la gestión de proyectos dentro de ISI Mustang?\\
  \textit{R:} Soy Jefe de Proyectos. Me encargo de la planificación, el seguimiento y el control de los proyectos asignados: armo el cronograma, coordino los recursos y el personal, hago el seguimiento del avance y de los costos, y reporto el estado a la gerencia. Básicamente soy el responsable de que el proyecto se entregue en plazo, en presupuesto y con la calidad comprometida.

  \item ¿Qué tipo de proyectos de ingeniería (EPC) gestiona habitualmente y cuál es su duración y envergadura típicas?\\
  \textit{R:} Trabajamos proyectos EPC para los sectores de gas y petróleo, energía y minería, sobre todo de fabricación y montaje. La duración varía bastante, normalmente entre seis y dieciocho meses, y son proyectos de presupuesto alto, donde una desviación pequeña en porcentaje ya representa montos importantes.

  \item ¿Cuáles son las decisiones más críticas que debe tomar durante la planificación y el seguimiento de un proyecto?\\
  \textit{R:} Las más críticas tienen que ver con la asignación de recursos y con los ajustes de cronograma cuando algo se atrasa. También las decisiones de compra y requisición de materiales, porque si una compra se demora arrastra a todo el proyecto.
\end{enumerate}

\noindent\textbf{Bloque II. Desviaciones y riesgos (el problema)}
\begin{enumerate}\setlength{\itemsep}{6pt}\setlength{\parskip}{0pt}
  \item ¿Qué tipos de desviaciones son las más frecuentes en sus proyectos: sobrecostos, retrasos en plazos, asignación de recursos o certificaciones?\\
  \textit{R:} Las más frecuentes son los retrasos en los plazos y los sobrecostos, y muchas veces van de la mano. Buena parte se origina en la asignación de recursos y en observaciones que aparecen en las certificaciones, que obligan a rehacer trabajo.

  \item ¿Cómo y en qué momento se detectan actualmente esas desviaciones?\\
  \textit{R:} Hoy las detectamos a través de los reportes del ERP y en las reuniones de avance, que son mensuales o quincenales. El problema es que para cuando el reporte muestra la desviación, esta ya ocurrió; estamos mirando hacia atrás.

  \item ¿La detección suele ser oportuna o más bien reactiva? ¿Qué consecuencias trae detectarlas de forma tardía?\\
  \textit{R:} Es bastante reactiva, esa es la verdad. Cuando se detecta tarde ya hay retrabajos, a veces penalidades por incumplimiento de plazos y sobrecostos que ya no se pueden recuperar. Reaccionamos en lugar de prevenir.

  \item Desde su experiencia, ¿qué causas originan con mayor frecuencia esas desviaciones?\\
  \textit{R:} Suelen ser estimaciones iniciales demasiado optimistas, cambios de alcance durante la ejecución, demoras en las requisiciones de compra y observaciones en certificación que generan reprocesos.
\end{enumerate}

\noindent\textbf{Bloque III. Uso del ERP y datos disponibles (la data)}
\begin{enumerate}\setlength{\itemsep}{6pt}\setlength{\parskip}{0pt}
  \item ¿Qué módulos del ERP utiliza con mayor frecuencia y para qué tareas?\\
  \textit{R:} Uso a diario los módulos de proyectos, certificaciones, el de horas y personal, y el de requisiciones y compras. Sobre todo para cargar y revisar avances, controlar costos y hacer seguimiento al estado de las compras.

  \item ¿Qué información registra y qué información consulta habitualmente en el sistema?\\
  \textit{R:} Registro avances, horas trabajadas, certificaciones y estados de requisición. Consulto principalmente los reportes de avance físico y de costos, y el estado de las certificaciones de cada proyecto.

  \item En el módulo de certificaciones, ¿cómo se registran las desviaciones y sus causas? ¿Qué campos se completan?\\
  \textit{R:} En certificaciones se registra la certificación con sus fechas, el responsable y, cuando hay una observación o desviación, se anota la causa en un campo destinado a eso. El dato existe, pero a veces la descripción de la causa queda muy breve o genérica, depende de quién la cargue.

  \item ¿Considera que el historial almacenado en el ERP refleja fielmente lo que realmente ocurre en los proyectos?\\
  \textit{R:} En lo general sí, la información troncal está. Pero el nivel de fidelidad depende mucho de la disciplina de carga de cada persona; hay detalles que se pierden porque no siempre se registran en el momento.
\end{enumerate}

\noindent\textbf{Bloque IV. Calidad y confiabilidad del registro (calidad de datos)}
\begin{enumerate}\setlength{\itemsep}{6pt}\setlength{\parskip}{0pt}
  \item ¿Con qué nivel de detalle y de consistencia se cargan los datos en el sistema?\\
  \textit{R:} Los datos clave, como fechas, montos y responsables, se cargan de forma bastante consistente. Donde hay más variabilidad es en los comentarios y en el detalle de las causas, que dependen del criterio de cada usuario.

  \item ¿Ha identificado inconsistencias, datos faltantes o registros duplicados en el historial?\\
  \textit{R:} Sí, ocurre. Hay campos que quedan vacíos, fechas que se cargan días después de lo real y, cuando se corrige un registro, a veces queda algún duplicado. No es masivo, pero está presente.

  \item ¿Qué tan estandarizado está el proceso de registro entre las distintas personas o áreas que alimentan el ERP?\\
  \textit{R:} Está parcialmente estandarizado. Tenemos formatos y procedimientos, pero la manera de describir las causas o de clasificar una observación no es uniforme entre todas las áreas.
\end{enumerate}

\noindent\textbf{Bloque V. Necesidades y expectativas del módulo predictivo}
\begin{enumerate}\setlength{\itemsep}{6pt}\setlength{\parskip}{0pt}
  \item ¿Qué predicciones le resultarían más útiles: anticipación de sobrecostos, de retrasos, de cuellos de botella en recursos u otras?\\
  \textit{R:} Las más útiles serían anticipar retrasos y sobrecostos, y poder ver con tiempo dónde se van a generar cuellos de botella en los recursos. Eso me permitiría redistribuir antes de que el problema golpee.

  \item ¿Con qué anticipación necesitaría recibir esas alertas para que le resulten verdaderamente accionables?\\
  \textit{R:} Idealmente con dos a cuatro semanas de anticipación. Con ese margen todavía puedo reasignar recursos, adelantar una compra o ajustar el cronograma; con menos tiempo ya es solo apagar incendios.

  \item ¿Cómo y dónde preferiría visualizar esta información: un tablero (dashboard), dentro del propio ERP, mediante notificaciones?\\
  \textit{R:} Lo ideal sería un tablero dentro del mismo ERP, con una vista tipo semáforo por proyecto, y que además me lleguen notificaciones cuando un indicador se ponga en riesgo. Que no tenga que entrar a buscarlo.

  \item ¿Qué criterios utilizaría para confiar en una predicción generada por el sistema?\\
  \textit{R:} Que haya acertado en casos pasados que yo conozco, que me explique en qué se basa la alerta y no sea una caja negra, y que sea consistente en el tiempo. La confianza se gana con aciertos.

  \item ¿Qué impacto esperaría que tuviera esta herramienta en su trabajo diario y en la gestión de los proyectos?\\
  \textit{R:} Esperaría pasar de una gestión reactiva a una preventiva. Si logramos anticiparnos, se reducen los sobrecostos y mejora el cumplimiento de plazos, y mi equipo trabaja con menos urgencias de último momento.
\end{enumerate}

\noindent\textbf{Bloque VI. Cierre}
\begin{enumerate}\setlength{\itemsep}{6pt}\setlength{\parskip}{0pt}
  \item ¿Desea añadir algún comentario, observación o sugerencia adicional respecto al tema tratado?\\
  \textit{R:} Solo que sea una herramienta fácil de usar y que no agregue carga de trabajo de carga de datos. Y como sugerencia, empezaría por el módulo de certificaciones, que es donde tenemos el historial más rico de desviaciones.
\end{enumerate}
